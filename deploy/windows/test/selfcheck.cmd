...
if exist tmp\selfcheck-certs rmdir /s /q tmp\selfcheck-certs
build\bin\server\lsyl-tunnel-cert.exe -out tmp\selfcheck-certs -hosts "localhost,127.0.0.1" || exit /b 1
if not exist tmp\selfcheck-certs\server.crt (echo [ERROR] Missing: tmp\selfcheck-certs\server.crt & exit /b 1)
if not exist tmp\selfcheck-certs\server.key (echo [ERROR] Missing: tmp\selfcheck-certs\server.key & exit /b 1)
rmdir /s /q tmp\selfcheck-certs
...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$targets=@('build\bin\client\lsyl-tunnel-client.exe','build\bin\client\lsyl-tunnel-client-gui.exe','build\bin\client\lsyl-tunnel-client-lite.exe','build\bin\profile\lsyl-tunnel-profile.exe','build\bin\server\lsyl-tunnel-server.exe','build\bin\server\lsyl-tunnel-server-gui.exe','build\bin\server\lsyl-tunnel-server-svc.exe','build\bin\server\lsyl-tunnel-cert.exe','build\bin\server\lsyl-tunnel-passwd.exe'); $bad=@(); foreach($t in $targets){ $vi=(Get-Item $t).VersionInfo; if([string]::IsNullOrWhiteSpace($vi.ProductName) -or [string]::IsNullOrWhiteSpace($vi.FileDescription) -or [string]::IsNullOrWhiteSpace($vi.CompanyName) -or $vi.FileVersion -ne '1.0.1.0'){ $bad += ($t + ' product=' + $vi.ProductName + ' desc=' + $vi.FileDescription + ' company=' + $vi.CompanyName + ' version=' + $vi.FileVersion) } }; if($bad){ Write-Host '[ERROR] Missing or incomplete Windows version resource:'; $bad | ForEach-Object { Write-Host ('  ' + $_) }; exit 1 }" || exit /b 1
...
powershell -NoProfile -ExecutionPolicy Bypass -File deploy\windows\sign\write-release-manifest.ps1 -OutputDir tmp\selfcheck-release-manifest -Files build\bin\client\lsyl-tunnel-client.exe,build\bin\client\lsyl-tunnel-client-gui.exe,build\bin\client\lsyl-tunnel-client-lite.exe,build\bin\profile\lsyl-tunnel-profile.exe,build\bin\server\lsyl-tunnel-server.exe,build\bin\server\lsyl-tunnel-server-gui.exe,build\bin\server\lsyl-tunnel-server-svc.exe,build\bin\server\lsyl-tunnel-cert.exe,build\bin\server\lsyl-tunnel-passwd.exe >nul || exit /b 1
if not exist tmp\selfcheck-release-manifest\release-manifest.json (echo [ERROR] Missing generated release manifest json & exit /b 1)
if not exist tmp\selfcheck-release-manifest\release-manifest.txt (echo [ERROR] Missing generated release manifest txt & exit /b 1)
rmdir /s /q tmp\selfcheck-release-manifest
if exist tmp\selfcheck-client-package rmdir /s /q tmp\selfcheck-client-package
call deploy\windows\app\package-client.cmd tmp\selfcheck-client-package >nul || exit /b 1
if not exist tmp\selfcheck-client-package\cert\server.crt (echo [ERROR] Missing: client package cert & exit /b 1)
if not exist tmp\selfcheck-client-package\bin\lsyl-tunnel-client-lite.exe (echo [ERROR] Missing: client package Lite exe & exit /b 1)
if exist tmp\selfcheck-client-package\bin\lsyl-tunnel-profile.exe (echo [ERROR] Client package must not include independent profile tool & exit /b 1)
if not exist tmp\selfcheck-client-package\make-installer.cmd (echo [ERROR] Missing: client package installer builder & exit /b 1)
if not exist tmp\selfcheck-client-package\installer\client.iss (echo [ERROR] Missing: client package Inno script & exit /b 1)
if not exist tmp\selfcheck-client-package\installer\Languages\ChineseSimplified.isl (echo [ERROR] Missing: client package installer language & exit /b 1)
powershell -NoProfile -ExecutionPolicy Bypass -Command "$hit=Select-String -Path 'tmp\selfcheck-client-package\installer\client.iss' -SimpleMatch 'taskkill' -List; if($hit){ Write-Host ('[ERROR] Client package installer must not force-kill processes: ' + $hit.Path + ':' + $hit.LineNumber); exit 1 }" || exit /b 1
findstr /c:"RollbackClientRuntimeFiles" tmp\selfcheck-client-package\installer\client.iss >nul || (echo [ERROR] Client package installer is missing rollback cleanup logic & exit /b 1)
if exist tmp\selfcheck-client-package\install-client-app.cmd (echo [ERROR] Client package should not include manual install script & exit /b 1)
if exist tmp\selfcheck-client-package\uninstall-client-app.cmd (echo [ERROR] Client package should not include manual uninstall script & exit /b 1)
if exist tmp\selfcheck-client-package\uninstall-client-app.ps1 (echo [ERROR] Client package should not include manual uninstall script & exit /b 1)
if exist tmp\selfcheck-client-package\bin\lsyl-tunnel-client-svc.exe (echo [ERROR] Client package should not include service exe & exit /b 1)
findstr /c:"../cert/server.crt" tmp\selfcheck-client-package\conf\client.yaml >nul || exit /b 1
powershell -NoProfile -ExecutionPolicy Bypass -Command "$p='tmp\selfcheck-client-package\conf\client.yaml'; $t=Get-Content -Raw -Encoding UTF8 $p; if($t -match 'ciphertext:|key_id:|client_id:\s*demo-client'){ Write-Host '[ERROR] Client package config contains runtime-only credential data or demo client id.'; exit 1 }; if($t -notmatch '(?m)^saved_credential:\s*\{\}' -or $t -notmatch '(?m)^forwards:'){ Write-Host '[ERROR] Client package config was not sanitized correctly or lost forwards.'; exit 1 }" || exit /b 1
rmdir /s /q tmp\selfcheck-client-package
if exist tmp\selfcheck-light-clients-package rmdir /s /q tmp\selfcheck-light-clients-package
call deploy\windows\app\package-light-clients.cmd tmp\selfcheck-light-clients-package >nul || exit /b 1
if not exist tmp\selfcheck-light-clients-package\android\lsyl-tunnel-mobile.apk (echo [ERROR] Missing: lightweight package Android APK & exit /b 1)
if not exist tmp\selfcheck-light-clients-package\windows-win7\lsyl-tunnel-client-lite.exe (echo [ERROR] Missing: lightweight package Win7 Lite exe & exit /b 1)
if not exist tmp\selfcheck-light-clients-package\profiles\README.txt (echo [ERROR] Missing: lightweight package profiles README & exit /b 1)
powershell -NoProfile -ExecutionPolicy Bypass -Command "$p='tmp\selfcheck-light-clients-package\windows-win7\lsyl-tunnel-client-lite.exe'; $b=[IO.File]::ReadAllBytes($p); $pe=[BitConverter]::ToInt32($b,0x3c); $machine=[BitConverter]::ToUInt16($b,$pe+4); if($machine -ne 0x014c){ Write-Host ('[ERROR] Lightweight package Win7 Lite client must be PE32/i386, got machine 0x{0:X4}' -f $machine); exit 1 }" || exit /b 1
rmdir /s /q tmp\selfcheck-light-clients-package
if exist tmp\selfcheck-profile-package rmdir /s /q tmp\selfcheck-profile-package
call deploy\windows\app\package-profile.cmd tmp\selfcheck-profile-package >nul || exit /b 1
if not exist tmp\selfcheck-profile-package\bin\lsyl-tunnel-profile.exe (echo [ERROR] Missing: profile package tool & exit /b 1)
if not exist tmp\selfcheck-profile-package\README.txt (echo [ERROR] Missing: profile package README & exit /b 1)
if exist tmp\selfcheck-profile-package\bin\lsyl-tunnel-client-gui.exe (echo [ERROR] Profile package must not include client GUI & exit /b 1)
if exist tmp\selfcheck-profile-package\bin\lsyl-tunnel-client-lite.exe (echo [ERROR] Profile package must not include client Lite & exit /b 1)
if exist tmp\selfcheck-profile-package\installer (echo [ERROR] Profile package must not include installer files & exit /b 1)
rmdir /s /q tmp\selfcheck-profile-package
if exist tmp\selfcheck-server-package rmdir /s /q tmp\selfcheck-server-package
call deploy\windows\app\package-server.cmd tmp\selfcheck-server-package >nul || exit /b 1
if not exist tmp\selfcheck-server-package\bin\lsyl-tunnel-server.exe (echo [ERROR] Missing: server package cli & exit /b 1)
if not exist tmp\selfcheck-server-package\bin\lsyl-tunnel-server-svc.exe (echo [ERROR] Missing: server package service & exit /b 1)
if not exist tmp\selfcheck-server-package\bin\lsyl-tunnel-server-gui.exe (echo [ERROR] Missing: server package gui & exit /b 1)
if not exist tmp\selfcheck-server-package\bin\lsyl-tunnel-cert.exe (echo [ERROR] Missing: server package cert tool & exit /b 1)
if not exist tmp\selfcheck-server-package\bin\lsyl-tunnel-passwd.exe (echo [ERROR] Missing: server package password tool & exit /b 1)
if not exist tmp\selfcheck-server-package\conf\server.yaml (echo [ERROR] Missing: server package config & exit /b 1)
if not exist tmp\selfcheck-server-package\certs (echo [ERROR] Missing: server package certs dir & exit /b 1)
if not exist tmp\selfcheck-server-package\data (echo [ERROR] Missing: server package data dir & exit /b 1)
if not exist tmp\selfcheck-server-package\logs (echo [ERROR] Missing: server package logs dir & exit /b 1)
if not exist tmp\selfcheck-server-package\assets\server.ico (echo [ERROR] Missing: server package icon & exit /b 1)
if not exist tmp\selfcheck-server-package\make-installer.cmd (echo [ERROR] Missing: server package installer builder & exit /b 1)
if not exist tmp\selfcheck-server-package\installer\server.iss (echo [ERROR] Missing: server package Inno script & exit /b 1)
if not exist tmp\selfcheck-server-package\installer\Languages\ChineseSimplified.isl (echo [ERROR] Missing: server package installer language & exit /b 1)
findstr /c:"autostartservice" tmp\selfcheck-server-package\installer\server.iss >nul || (echo [ERROR] Server package installer is missing service auto-start confirmation task & exit /b 1)
findstr /c:"-start-type" tmp\selfcheck-server-package\installer\server.iss >nul || (echo [ERROR] Server package installer is missing service start-type registration parameter & exit /b 1)
if exist tmp\selfcheck-server-package\install-server-app.cmd (echo [ERROR] Server package should not include manual install script & exit /b 1)
if exist tmp\selfcheck-server-package\uninstall-server-app.cmd (echo [ERROR] Server package should not include manual uninstall script & exit /b 1)
if exist tmp\selfcheck-server-package\uninstall-server-app.ps1 (echo [ERROR] Server package should not include manual uninstall script & exit /b 1)
findstr /c:"../certs/server.crt" tmp\selfcheck-server-package\conf\server.yaml >nul || exit /b 1
findstr /c:"../data/server-state.json" tmp\selfcheck-server-package\conf\server.yaml >nul || exit /b 1
powershell -NoProfile -ExecutionPolicy Bypass -Command "$p='tmp\selfcheck-server-package\conf\server.yaml'; $t=Get-Content -Raw -Encoding UTF8 $p; if($t -match '(?m)username:\s*(alice|bob)\b' -or $t -match '(?m)^\s*-\s*name:\s*(web|sql|rev)\b'){ Write-Host '[ERROR] Server package config contains demo users or demo forwards.'; exit 1 }" || exit /b 1
rmdir /s /q tmp\selfcheck-server-package
