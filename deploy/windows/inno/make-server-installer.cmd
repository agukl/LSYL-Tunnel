...
set "OUT_FILE=%OUT_DIR%\LSYL-Tunnel-Server-Setup.exe"

if exist "%OUT_FILE%" (
  del /f /q "%OUT_FILE%" >nul 2>nul
  if exist "%OUT_FILE%" (
    echo [ERROR] Cannot remove old server installer:
    echo   %OUT_FILE%
    exit /b 1
  )
)
...
if not exist "%PACKAGE_DIR%bin\lsyl-tunnel-server-svc.exe" (
  echo [ERROR] Missing server service binary:
  echo   %PACKAGE_DIR%bin\lsyl-tunnel-server-svc.exe
  exit /b 1
)
if not exist "%PACKAGE_DIR%bin\lsyl-tunnel-server-gui.exe" (
  echo [ERROR] Missing server GUI:
  echo   %PACKAGE_DIR%bin\lsyl-tunnel-server-gui.exe
  exit /b 1
)
if not exist "%PACKAGE_DIR%conf\server.yaml" (
  echo [ERROR] Missing server config:
  echo   %PACKAGE_DIR%conf\server.yaml
  exit /b 1
)
...
echo [INFO] Building server installer from package:
echo   %PACKAGE_DIR%
echo [INFO] Inno compiler:
echo   %ISCC%
"%ISCC%" "/O%OUT_DIR%" "%SCRIPT_FILE%" || exit /b 1
if not exist "%OUT_FILE%" (
  echo [ERROR] Server installer was not created:
  echo   %OUT_FILE%
  exit /b 1
)
