...
服务端会维护 `credential_seal` 的激活密钥。当前密钥过期后，服务端会切换到可用的新密钥或生成新的密钥；旧客户端保存的 `saved_credential` 按原过期时间自然失效。
...
服务端 GUI 中，“用户认证”页只维护账号；“转发端口”页按用户分配目标放通。正向服务端目标仅允许回环地址或私有 IP:端口，且必须与规则精确匹配；反向端口固定为服务端回环地址，每个端口只能归属一个账号，同一账号可以拥有多个反向端口。客户端必须在反向请求中提交配置的 `listen_addr`，服务端按“监听地址 + 登录账号”校验端口归属，但不会向客户端下发或改写该地址。规则名不参与授权。核心校验不会接受全局放行策略、公网 IP、域名或任意未配置目标。
