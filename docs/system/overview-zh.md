...
安装、签名和发版流程见 [Windows 部署与安装](../deployment/windows-deployment-zh.md) 和 [发版手册](../release/release-process-zh.md)。
...
7. 对每条正向转发建立固定本地监听，或在管理员授权后用 WinDivert 接管证书 IP 的指定业务端口并反射到独立的随机回环端口；对每条反向端口建立反向控制连接。
