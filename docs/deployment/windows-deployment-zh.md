...
Windows 虚拟转发模式在每次登录成功后按规则请求管理员授权，由独立提权子进程加载随客户端安装且通过固定 SHA-256 校验的 WinDivert 2.2.2，只过滤证书 IPv4 SAN 与配置业务端口组成的端点，并桥接到随机 `127.0.0.1` 端口。它不添加网卡地址、不修改路由，也不处理同一 IP 的其他端口；单条规则取消授权或异常不会关闭其他规则。客户端退出或异常终止后，过滤句柄随进程关闭，因此安装器不需要维护或清理虚拟 IP 状态文件。安装包同时携带 WinDivert 许可和对应源代码归档；Win7 Lite 与 Android 不加载这些组件。
