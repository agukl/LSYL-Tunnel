...
- `server_to_client`：服务端创建被动入口，客户端连接目标。服务端入口必须由已认证客户端主动连接后激活，服务端不会主动连接客户端。反向规则只使用被动端口，不配置 IP。
...
- 正向 `client_to_server`：GUI 分别填写服务端 IP 和端口，保存时组合为 `server_target`。可填写回环地址或私有 IP，例如 `127.0.0.1`、`192.168.10.20`；组合后的目标必须与客户端配置中的 `server_target` 完全一致。保存时会预检目标格式，服务启动后会再次审计并记录运行事件。目标临时不可访问不拖垮服务主入口，实际连接时会返回“目标服务不可达”。
- 反向 `server_to_client`：服务启动时只加载配置并审计端口状态；客户端上线激活时才会尝试监听服务端被动端口。端口临时被占用时不拖垮服务端，客户端会自动重试并记录“服务端被动端口不可用”。
- 反向端口运行时固定绑定服务端本机回环地址 `127.0.0.1:<端口>`，不支持也不保存网卡 IP。
- 反向端口不能重复，并且必须只归属一个已存在账号；同一账号可以绑定多个反向端口。账号停用时不能登录和激活端口，重新启用后仍沿用原归属。GUI 页面和服务端配置文件只填写 `listen_port`，并把所选账号写入 `allowed_users`。
- 客户端必须提交反向规则的 `listen_addr` 来选择服务端端口，不通过规则名选择。服务端校验该监听地址已配置且归属登录账号；协议响应不会返回端口号或监听地址，也不会改写客户端配置。
...
`monitor_addr` 只能使用服务端回环地址，例如 `127.0.0.1`、`::1` 或 `localhost`。监控接口包含运行状态和封禁管理操作，不支持绑定到公网或内网网卡。
